...
class Amazon(NodeDataset):
    def __init__(self, name="photo", root="./", split="random", k=None):
        if name not in ['computers', 'photo']:
            raise ValueError("Dataset name not supported!")
        super(Amazon, self).__init__(root + "amazon/", name, k)
    ...
    def raw_file_paths(self):
        return osp.join(self.raw_dir, f'amazon_electronics_{self.name.lower()}.npz')
    ...
    def process(self):
        ...
        g = Graph(row, col, edge_weight, num_node, edge_type, x=x.numpy(), y=y)
